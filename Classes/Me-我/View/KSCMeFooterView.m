...
#import "KSCSqaureButton.h"
#import "KSCWebViewController.h"

@implementation KSCMeFooterView
...
- (instancetype)initWithFrame:(CGRect)frame
{
    if (self = [super initWithFrame:frame]) {
        ...
        [[AFHTTPSessionManager manager] GET:@"http://api.budejie.com/api/api_open.php" parameters:params progress:^(NSProgress * _Nonnull downloadProgress) {
            ...
            //            NSArray *sqaures = [KSCSquare objectArrayWithKeyValuesArray:responseObject[@"square_list"]];
...
- (void)createSquares:(NSArray *)sqaures
{
    ...
    // 宽度和高度
    CGFloat buttonW = KSCScreenW / maxCols;
    CGFloat buttonH = buttonW;
    
    for (int i = 0; i<sqaures.count; i++) {
        // 创建按钮
        KSCSqaureButton *button = [KSCSqaureButton buttonWithType:UIButtonTypeCustom];
        // 监听点击
...
- (void)buttonClick:(KSCSqaureButton *)button
{
    if (![button.square.url hasPrefix:@"http"]) return;
    
    KSCWebViewController *web = [[KSCWebViewController alloc] init];
